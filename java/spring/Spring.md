...
- 代理模式：Spring的Aop功能用到了JDK的动态代理类和CGLIB字节码生成技术；

- 模板方法：解决重复代码问题；如：RestTemplate,JpaTemplate;
...
​	是个接口，**是IOC容器的核心接口，它的职责包括：实例化、定位、配置应用程序中的对象及建立这些对象间的依赖**；

### *ApplicationContext*：

​	接口,它由BeanFactory接口派生而来，ApplicationContext包含BeanFactory的所有功能，优先使用BeanFactory
...
BeanFactory采用的是延迟加载形式来注入Bean，只有在使用到某个Bean时（调用getBean()）才对该Bean进行加载实例化;

缺点：不能在程序启动时发现Spring中的配置问题；

ApplicationContext 在容器启动时，一次性创建了所有的Bean。可以及时发现Spring中的配置问题；

缺点：当程序配置Bean较多时，程序启动较慢；
